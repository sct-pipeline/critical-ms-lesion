...
def plot_laterality(laterality_report_folder, lesion_mask, lesion_statistics, asymmetry_csv, output_path):
    """
    ...
    Input:
        ...
        lesion_mask: Path to the lesion mask (NIfTI format)
        lesion_statistics: List of dictionaries containing lesion statistics
        asymmetry_csv: Path to the CSV file containing asymmetry data
        output_path: Path to the output folder
    Output:
        None
    """
    ...
    df_laterality = pd.DataFrame(columns=["Slice (I->S)", "lesion_label", "white matter", "gray matter", "dorsal columns", "lateral funiculi", "ventral funiculi", "total % (all tracts)"])
    ...
    for lesion in lesion_statistics:
        ...
        df_laterality = pd.concat([df_laterality, df_lesion[["Slice (I->S)", "lesion_label", "white matter", "gray matter", "dorsal columns", "lateral funiculi", "ventral funiculi", "total % (all tracts)"]]], ignore_index=True)
    
    # Now using the asymmetry csv file, we can add the VertLevel column: where slices indexes match
    df_asymmetry = pd.read_csv(asymmetry_csv)
    df_laterality = df_laterality.merge(df_asymmetry[["Slice (I->S)", "VertLevel"]], on="Slice (I->S)", how="left")
...
def detect_critical_lesions(input_scan, output_path, pam_50_csa_folder, path_hc_data):
    ...
    laterality_plot = plot_laterality(laterality_report_folder, lesion_mask, lesion_statistics, asymetry_csv, output_path)
