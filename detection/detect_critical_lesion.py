...
def run_lesion_segmentation(input_scan, output_path, run=True):
    ...
    # Run the SCT command
    if not run or os.path.exists(lesion_mask):
        return lesion_mask
...
def run_sc_segmentation(input_scan, output_path, run=True):
    ...
    # Run the SCT command
    if not run or os.path.exists(sc_mask):
        return sc_mask
...
def run_vert_labeling(input_scan, output_path, run=True):
    ...
    if not run or os.path.exists(vert_levels):
        return vert_levels
...
def compute_normalized_csa(sc_mask, vert_levels, output_path, run=True):
    ...
    if not run or os.path.exists(output_csv):
        return output_csv
...
def detect_critical_lesions(input_scan, output_path, pam_50_csa_folder):
    ...
    # Lesion segmentation
    lesion_mask = run_lesion_segmentation(input_scan, output_path, run=True)
    # SC segmentation
    sc_mask = run_sc_segmentation(input_scan, output_path, run=True)
    # Vert labeling
    vert_levels =run_vert_labeling(input_scan, output_path, run=True)
