...
plot_subject_csa() also accepts hemi=True, which plots CSA_left_mm2/CSA_right_mm2 as two
stacked panels (left hemicord on top, right at the bottom) instead of the single CSA_mm2 panel.
See plot_subject_csa_hemi.py for the standalone entry point that uses this.
...
def smooth_csa_columns(df, smooth_window, columns):
    """
    Applies a centered moving-average to the given columns, per session. Edge slices that don't
    have a full window available keep their raw (unsmoothed) value, instead of being averaged
    ...
    for session_id, df_session in df.groupby("session_id", sort=False):
        ...
        for column in columns:
            raw = df_session[column]
            smoothed = raw.rolling(window=smooth_window, center=True, min_periods=smooth_window).mean()
            df_session[column] = smoothed.fillna(raw)
        smoothed_sessions.append(df_session)

    return pd.concat(smoothed_sessions, ignore_index=True)


def plot_csa_panel(ax, df, sessions, palette, csa_column, title):
    """
    Plots one CSA-vs-slice panel (lesion shading, one line per session, vertebral level
    boundaries/labels) onto ax, for the given CSA column.
    """
    ...
    for session_idx, session_id in enumerate(sessions):
        ...
        sns.lineplot(ax=ax, x="pam50_axial_slice", y=csa_column, data=df_session, linewidth=2,
...
def plot_subject_csa(input_csv, output_png, smooth_window=1, hemi=False):
    ...
    # Sessions sorted chronologically (session_id is e.g. "ses-YYYYMMDD", so a lexical sort is
    # also a chronological sort) so the color gradient tracks time.
    sessions = sorted(df["session_id"].unique())
    n_sessions = len(sessions)
    palette = [cm.viridis(1 - i / max(n_sessions - 1, 1)) for i in range(n_sessions)]

    if hemi:
        df = smooth_csa_columns(df, smooth_window, ["CSA_left_mm2", "CSA_right_mm2"])
        fig, (ax_left, ax_right) = plt.subplots(2, 1, figsize=(16, 16))
        plot_csa_panel(ax_left, df, sessions, palette, "CSA_left_mm2",
                       f'Left hemicord CSA across PAM50 axial slices and vertebral levels for {subject_id}')
        plot_csa_panel(ax_right, df, sessions, palette, "CSA_right_mm2",
                       f'Right hemicord CSA across PAM50 axial slices and vertebral levels for {subject_id}')
        fig.tight_layout()
    else:
